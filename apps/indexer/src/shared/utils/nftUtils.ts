...
type TxEventType =
  ...
  | "wasm-payout-seller"
  | "wasm-refund-bidder";
export function getEventAttributeValue(events: TransactionEventWithAttributes[], eventType: TxEventType, attributeKey: string, index= 0) {
  const event = events.find((event) => event.type === eventType && event.index === index);
